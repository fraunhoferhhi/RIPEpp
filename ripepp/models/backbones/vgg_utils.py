...
class VGG19(nn.Module):
    def __init__(self, pretrained=False, num_input_channels=3) -> None:
        ...
        self.layers = nn.ModuleList(
            tvm.vgg19_bn(weights=VGG19_BN_Weights.IMAGENET1K_V1 if pretrained else None).features[:40]
        )
...
class VGG(nn.Module):
    def __init__(self, num_layers=4) -> None:
        ...
        assert num_layers in [1, 2, 3, 4], "num_layers must be in [1, 2, 3, 4]"
        self.num_layers = num_layers

        self.layers = nn.ModuleList(tvm.vgg19_bn(pretrained=True).features[:40])

        # Maxpool layers: 6, 13, 26, 39
        location_max_pool_layers = {
            1: 6,
            2: 13,
            3: 26,
            4: 39,
        ...
        self.layers = self.layers[: location_max_pool_layers[num_layers] + 1]

    def get_dim_layers(self):
        return [64, 128, 256, 512][: self.num_layers]

    def get_subsampling_rates(self):
        return [1, 2, 4, 8][: self.num_layers]
