...
class VGG(BackboneBase):
    def __init__(self, nchannels=3, pretrained=True, use_instance_norm=True, mode="dect"):
        ...
        log.info(f"Using VGG encoder with mode {mode}")
    ...
    def _forward(self, x):
        encoder_layers, sizes = self.encoder(x)
        output = 0
        context = None
        scales = self.decoder.scales
        for idx, (feature_map, scale) in enumerate(zip(reversed(encoder_layers), scales, strict=False)):
        ...
        if self.mode == "dect":
            return {"heatmap": output, "encoder_descs_layers": encoder_layers}
        elif self.mode == "desc":
            return {"fine_descs": output, "encoder_descs_layers": encoder_layers}
            ...
            return {
                "heatmap": logits,
                "decoder_descs_map": descs,
                "encoder_descs_layers": encoder_layers,
            }
        else:
            raise ValueError("mode should be 'dect', 'desc' or 'dect+desc'")
