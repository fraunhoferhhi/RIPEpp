...
from ripepp.data.datasets.disk_imw import DISK_IMW
...
class IMW_2020_Benchmark:
    def __init__(
        ...
        self.root_path = Path(data_dir) / "disk-data"
        ...
        # fmt: off
        if use_predefined_subset or dry_run:
            ...
            if dry_run:
                # For dry run, limit the number of samples to a small subset
                log.info("Running in dry run mode, limiting to 10 samples.")
    ...
    def plot_results(self, num_samples=10, logger=None, step=None, downsampled=False, use_plt=False):
        if len(self.results) == 0:
    ...
    def log_results(self, logger=None, step=None, downsampled=False):
        ...
        summary_results = self.calc_auc(downsampled=downsampled)
        summary_results["ransac_inl_ratio"] = self.calc_metric("ransac_inl_ratio")
    ...
    def print_results(self):
        ...
        print(self.calc_auc())
        print(self.calc_metric("ransac_inl_ratio"))
